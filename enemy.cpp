...
void drawsecondEnemy()
{
	for (int i = 0; i < EnemyNum; i++) {
		if (1 <= kisidahp && kisidahp <= 100 && scene == second) {
			DrawFormatString(300, 30, GetColor(25, 200, 0), "vs岸田：HP%d", kisidahp);
		}
		if (enemy2[i].enable == true) {
			DrawGraph(enemy2[i].x - 10, enemy2[i].y - 40, kisidaimg, true);
		}
		if (scene == second && 75 <= kisidahp && kisidahp <= 100) {
			DrawFormatString(200, 550, GetColor(255, 0, 0), "所詮下級戦士如きがこの第１００代内閣総理大臣\n岸田文雄を倒せるとでも思っているのか");
		}
		if (scene == second && 50 <= kisidahp && kisidahp <= 74) {
			DrawFormatString(200, 550, GetColor(255, 0, 0), "どうだ怖気着いたか？ならばくらえ！！HigherTax!!");
			//HigherTax 訳　増税
